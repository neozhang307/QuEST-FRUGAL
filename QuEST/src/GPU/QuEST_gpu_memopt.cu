...
// Adapter layer between FRUGAL's array-based memory management and QuEST's Qureg structure
// FRUGAL manages 16 individual arrays (shards), QuEST kernels expect a Qureg struct containing all shards
// Direct mapping: arrayId in FRUGAL = shard index in QuEST (0-15)
namespace memopt_adapter {

template <typename T>
void allocateShardAndRegister(T** p, size_t s) {
  // Use FRUGAL's configuration directly
  ...
  // Register directly with FRUGAL's MemoryManager
  auto& memManager = memopt::MemoryManager::getInstance();
  memManager.registerManagedMemoryAddress(*p, s);
  memManager.registerApplicationInput(*p);
  memManager.registerApplicationOutput(*p);
}
...
void registerAndExecuteTask(
  ...
  for (auto shardIdx : ioShardIndices) {
    // Direct mapping: shard index = arrayId in FRUGAL
    // Each shard's real array is registered as a separate array in FRUGAL
    void* shardArray = qureg.deviceStateVecShards[shardIdx].real;
    inputs.push_back(shardArray);
    outputs.push_back(shardArray);
  }
...
template <typename T>
void moveDataBackToDevice(T*& deviceAddress, const std::map<void*, void*>& managedDeviceArrayToStorageArrayMap, size_t numAmpsPerShard) {
  // Check if address was actually moved to storage
  if (managedDeviceArrayToStorageArrayMap.count(deviceAddress) == 0) {
    return;  // Data wasn't moved to storage, no action needed
  }
  
  // Get storage address and copy data back to device
  auto storageAddress = managedDeviceArrayToStorageArrayMap.at(deviceAddress);
  checkCudaErrors(cudaMalloc(&deviceAddress, 2 * numAmpsPerShard * sizeof(qreal)));
  checkCudaErrors(cudaMemcpy(deviceAddress, storageAddress, 2 * numAmpsPerShard * sizeof(qreal), cudaMemcpyDefault));
  
  // Free storage memory based on allocation type
...
void applyFullQFTWithMemopt(Qureg* qureg) {
  ...
  if (memopt::ConfigurationManager::getConfig().generic.optimize) {
    ...
    float runningTime;
    std::map<void*, void*> managedDeviceArrayToStorageArrayMap;
    memopt::executeOptimizedGraph(
      ...
      runningTime,
      managedDeviceArrayToStorageArrayMap
    );

    printf("Total time used (s): %.6f\n", runningTime);

    // Restore shard data from storage back to device if needed
    for (StateVecIndex_t i = 0; i < qureg->numShards; i++) {
      memopt_adapter::moveDataBackToDevice(qureg->deviceStateVecShards[i].real, managedDeviceArrayToStorageArrayMap, qureg->numAmpsPerShard);
      qureg->deviceStateVecShards[i].imag = qureg->deviceStateVecShards[i].real + qureg->numAmpsPerShard;
    }
